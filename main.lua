...
if suc and type(web) ~= "boolean" then
    ...
    if game.GameId == 2619619496 then
        ...
        if game.PlaceId == 11807355973 then
            local bedwars = {
                ["sprintTable"] = KnitClient.Controllers.SprintController,
            }
            local sprintconnection
            local FastHeal = addModule("FastHeal", "Dont wait long to heal.", function(callback)
                if callback then
game:GetService("ReplicatedStorage").Common.Library.Network.RemoteEvent:FireServer("S_Popup",{workspace.World3.__HEART})
                end
            end)
        else
    ...
    local function connectionclose()
        ...
        task.spawn(function()
            if shared.noinject == nil then
                ...
                loadstring(readfile("soapclientlite.txt"))()
            end
        end)
    end
